...
def _get_hole_location_container_to_cobot_str(cont_in):
    ...
    for ahl in cont_in.locations:
        ...
        str_ = str_ + UID_TAG + cont_in.uid + CLOSE_TAG
...
def _get_pose_str(pose_in):
    ...
    # Get position
    str_ = str_ + POSE_PX_TAG + str(pose_in.position.x) + CLOSE_TAG
    str_ = str_ + POSE_PY_TAG + str(pose_in.position.y) + CLOSE_TAG
    str_ = str_ + POSE_PZ_TAG + str(pose_in.position.z) + CLOSE_TAG

    # Get orientation
    str_ = str_ + POSE_OX_TAG + str(rot_vect[0]) + CLOSE_TAG
    str_ = str_ + POSE_OY_TAG + str(rot_vect[1]) + CLOSE_TAG
    str_ = str_ + POSE_OZ_TAG + str(rot_vect[2]) + CLOSE_TAG
...
def _get_material_layer_to_cobot_str(layer_in):
    ...
    str_ = str_ + UID_TAG + layer_in.uid + CLOSE_TAG

    # Thickness of the layer:float32 thickness 
    str_ = str_ + LAYER_THICKNESS_TAG + str(layer_in.thickness) + CLOSE_TAG

    # Drill specifications during the drilling in this layer
    # Speed of the drill:float32 speed     
    str_ = str_ + DRILL_SPEED_TAG + str(layer_in.speed) + CLOSE_TAG  

    # Feed rate of the drill:float32 feed  
    str_ = str_ + DRILL_FEED_TAG + str(layer_in.feed) + CLOSE_TAG 

    # Lower limit(s) of the torque bandwidth(s):float32 lower_torque_limits  
    str_ = str_ + LOWER_TORQUE_LIMIT_TAG + str(layer_in.lower_torque_limits) + CLOSE_TAG

    # Upper limit(s) of the torque bandwidth(s):float32 upper_torque_limits  
    str_ = str_ + UPPER_TORQUE_LIMIT_TAG + str(layer_in.upper_torque_limits) + CLOSE_TAG  

    # Torque level/threshold to switch to the next layer in the stack: float32 threshold 
    str_ = str_ + TORQUE_THRESHOLD_TAG + str(layer_in.threshold) + CLOSE_TAG 

    # Temporary fastening specifications for this layer
    # Maximum temporary fastener clamp force:float32 max_clamp_force                 
    str_ = str_ + MAX_TEMPF_CLAMP_FORCE_TAG + str(layer_in.max_clamp_force) + CLOSE_TAG
...
def _get_waypoint_to_cobot_str(waypoint_in):
    ...
    str_ = str_ + UID_TAG + waypoint_in.uid + CLOSE_TAG

    # pos: location of the waypoint
    str_ = str_ + _get_pose_str(waypoint_in.pos)
...
def _get_action_to_cobot_str(action_in):
    ...
    str_ = str_ + UID_TAG + action_in.uid + CLOSE_TAG
...
def _get_fastener_to_cobot_str(fastener_in):
    ...
    str_ = str_ + END_EFFECTOR_UID_TAG + fastener_in.ee_uid + CLOSE_TAG

    #AssemblyFastState state                 # The state of the fastener
    str_ = str_ + FASTENER_STATE_TAG + str(fastener_in.state) + CLOSE_TAG

    #geometry_msgs/Pose inst_pos             # Installed location of the fastener
                                             # Only available after installation
    str_ = str_ + _get_pose_str(fastener_in.inst_pos)

    #float32 diam                            # diameter of the fastener
    str_ = str_ + DIAM_TAG + str(fastener_in.diam) + CLOSE_TAG

    #float32 shaft_height                    # the height of the shaft that is sticking out when in storage location
    str_ = str_ + SHAFT_HEIGHT_TAG + str(fastener_in.shaft_height) + CLOSE_TAG

    #float32 min_stack                       # the minimum stack
    str_ = str_ + MIN_STACK_TAG + str(fastener_in.min_stack) + CLOSE_TAG

    #float32 max_stack                       # the maximum stack
    str_ = str_ + MAX_STACK_TAG + str(fastener_in.max_stack) + CLOSE_TAG

    #float32 tcp_tip_distace                 # distance between hole entry point and tip if inserted in a hole
    str_ = str_ + TCP_TIP_DIST_TAG + str(fastener_in.tcp_tip_distace) + CLOSE_TAG
